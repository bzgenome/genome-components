...
export class BzgComboComponent implements ControlValueAccessor, OnInit {
  ...
  localFilter = false;
  @Input()
  localFilterCaseSensitive = true;
  @Input()
  typeAheadDelay = 500;
  @Input()
  inputClass = 'bzg-combo__input';
  @Input()
  inputPlaceholder = '';
  @Input()
  loadingIconClass = 'bzg-combo__icons__loader';
  @Input()
  triggerIconClass = 'bzg-combo__icons__trigger';
  @Input()
  dataRoot = '';
  @Input()
  ...
  @Output() initValue = new EventEmitter<string>();
  ...
  private scrollToMarkedElement() {
    const el = this.scrollElement.nativeElement.querySelector(`.bzg-combo__list__item:nth-child(${this.marked + 1})`);
  ...
  onItemClick(index: number, item: Object) {
    ...
    this.select.emit(this.data[this.marked]);
    ...
    if (!this.loading) {
      ...
      if (null === this.marked) {
        if (this.forceSelection) {
          ...
          this.create.emit(this.currVal);
          // this may causes error
          this.sendModelChange(this.currVal);
        }
      } else {
        const item = this.data[this.marked];
        if (this.isDisabled(item)) {
          return;
        }
        this.select.emit(this.data[this.marked]);
